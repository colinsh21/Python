...
class Space(object):
    ...
    def __init__(self,graph=nx.grid_graph(dim=[3,3]) ,num_ants=2,num_colonies=1,
                 ...
                 links=[(1,0,(1,1))],capacities=[[1,5],[5,10]],edge_capacity=20, 
...
#                               edge_color='#111111')
        ...
        plt.title(r'Survivability$={:1.2f}$, Complexity$={:6.2f}$, sat_flow={}'.format(score[0],score[1],sat_flow),
                  fontsize=15)
    ...
    def visualize_pareto(self, gen_history, pareto_history):
        ...
        ax1.set_ylim([-100,max(y_p)*1.2])#min(y_p)*.1
                ...
                while (e<self.local_search_params[1]) and added :#added:
                    ...
                    for i in range(0,2):
                     #systems
                        ...
                        #if (test_ant[0].number_of_edges()==0) or (test_ant[1].number_of_edges()==0):
                            #sat_flow=self.flow_check(test_ant)
                            #print 'broken graph'                    
                            #break
                        #else:
...
if __name__=='__main__':
    ...
    for i in xrange(len(ant_graphs)):
        score=ant_graphs[i][-1]
