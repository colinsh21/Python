...
#                
#                # sink support
#                if cap>=st_dict[v]['t']: #installing >= total required
...
#                    heuristic_value+=0.0 #1.0/(1.0+abs(cap_supported-cap))
            ...
            #out_graph[u][v]['capacity_u']='{}'.format(self.capacity_to_add)
        
        for n in out_graph.nodes():
            out_graph.node[n]['n_label']='{}'.format(n)
...
class Space(object):
    ...
    def __init__(self,graph=nx.grid_graph(dim=[3,3]) ,num_ants=2,num_colonies=1,
                 ...
                 fraction_removals=4.0/9,removal_trials=10,local_search_params=(0,0),
        ...
        #Set up isomorphism check
        self.em=iso.numerical_edge_match('capacity',0)
        self.nm=iso.categorical_node_match('n_label','(0,0)')
        
        #dictionary of evaluated systems and their ID's        
        self.sys_lib=[{},{}] #ID:sys
        #dictionary of ID combos and their respective score
        self.Composite=namedtuple("Composite",["sys_1","sys_2"])        
        self.score_lib={} # Composite(ID_1,ID_2):score
...
#            complexity_score*=1/(flow_score_ini[0]*flow_score_ini[1])**3
    ...
    def check_score_graph(self,graphs):
        """
        Checks if the graph composite exists in dictionary
        """
        check=[0,0,0] #sys_1,sys_2,composit
        ID=[0,0]
        g=[copy.deepcopy(graphs[0]),copy.deepcopy(graphs[1])]
        #print self.sys_lib[0]
        #print self.sys_lib[1]
        for i in xrange(2): #iterate through sytems
            for g_ID in self.sys_lib[i]:
                g_test=copy.deepcopy(self.sys_lib[i][g_ID])                
#                print g, nx.is_isomorphic(g,graphs[i],edge_match=self.em)
                if nx.is_isomorphic(g_test,g[i],node_match=self.nm,edge_match=self.em): #check if the same
                    #print 'match',i,g_ID                    
                    ID[i]=g_ID
                    check[i]=1
        ...
        if check==[1,1,0]:
            comp=self.Composite(sys_1=ID[0],sys_2=ID[1])
            if comp in self.score_lib:
                check[2]=1
#                print 'first find',ID,self.score_lib[comp]['score']
            
        if check==[1,1,1]: #previous evaluated
#            print 'found',ID,check
            #print self.score_lib
            #exists=1
            self.repeats+=1
            comp=self.Composite(sys_1=ID[0],sys_2=ID[1])
            graph_score=tuple(self.score_lib[comp])
#            print comp
#            graph_score=tuple(self.score_lib[comp]['score'])
#            ret_g=self.score_lib[comp]['graphs']
            #check
#            cap_score_1, infeasible_e_count=self.cap_score(graphs)
#            g_0=self.sys_lib[0][ID[0]]
#            g_1=self.sys_lib[1][ID[1]]
#            cap_score_2, infeasible_e_count=self.cap_score([g_0,g_1])
#            t_0= nx.is_isomorphic(g_0,ret_g[0],node_match=self.nm,edge_match=self.em)
#            t_1= nx.is_isomorphic(g_1,ret_g[1],node_match=self.nm,edge_match=self.em)
#            print 'isos', t_0,t_1
#            print 'scores',cap_score_1,cap_score_2,graph_score[1],cap_score_1==graph_score[1]
            
            #find score_lib ID?
#            for c,score in self.score_lib.iteritems():
#                if score[1]==cap_score_1:
#                    g_0=self.sys_lib[0][c.sys_1]
#                    g_1=self.sys_lib[1][c.sys_2]
#                    t_0= nx.is_isomorphic(g_0,graphs[0],node_match=self.nm,edge_match=self.em)
#                    t_1= nx.is_isomorphic(g_1,graphs[1],node_match=self.nm,edge_match=self.em)
#                    if t_0==True and t_1==True:
#                        print 'actual',c
...
#            print 'original'
#            self.visualize_system_single(graphs,
#                                      space.source,space.source_magnitude,
#                                      space.sink,space.sink_magnitude,
#                                      space.g,(0,cap_score_1),7)
#                                      
#            print 'returned'                          
#            self.visualize_system_single(ret_g,
#                                      space.source,space.source_magnitude,
#                                      space.sink,space.sink_magnitude,
#                                      space.g,(1,graph_score[1]),7)
            #print ID,graph_score
        
        elif check==[1,1,0]: #graphs exist, new combo
            
            graph_score,sat_flow=self.evaluate_graph(g)
            comp=self.Composite(sys_1=ID[0],sys_2=ID[1])
#            print 'combo',ID,comp,graph_score
            self.score_lib[comp]=tuple(graph_score)
#            self.score_lib[comp]={}
#            self.score_lib[comp]['score']=tuple(graph_score)
#            self.score_lib[comp]['graphs']=graphs
#            print 'combo check',self.score_lib[comp]['score']
#            print 'original'
#            self.visualize_system_single(graphs,
#                                      space.source,space.source_magnitude,
#                                      space.sink,space.sink_magnitude,
#                                      space.g,ID,7)
            ...
            for i in xrange(2):
                if check[i]==0:
                    if self.sys_lib[i]:
                        max_id=max(self.sys_lib[i].keys(), key=int)
                        ID[i]=int(max_id+1)
                    else:
                        ID[i]=0
                    #add g to sys_lib
#                    print 'adding',i,ID[i]
                    self.sys_lib[i][ID[i]]=g[i]
            comp=self.Composite(sys_1=ID[0],sys_2=ID[1])
            self.score_lib[comp]=tuple(graph_score)
#            self.score_lib[comp]={}
#            self.score_lib[comp]['score']=tuple(graph_score)
#            self.score_lib[comp]['graphs']=graphs
#            print 'comp added',ID,comp,graph_score,self.score_lib[comp]['score']
...
#            graph_score=tuple(self.score_lib[comp])
#        else:
#            graph_score,sat_flow=self.evaluate_graph(graphs)
#            for check_index in xrange(len(check)):
#                if check[check_index]==0:
#                    if self.sys_lib[check_index]:
#                        max_id=max(self.sys_lib[check_index].keys(), key=int)
#                        ID[check_index]=max_id+1
#                    else:
#                        ID[check_index]=0
#            comp=self.Composite(sys_1=ID[0],sys_2=ID[1])
#            self.score_lib[comp]=graph_score          
        
        
        return graph_score #ID,check,exists 
...
#                                      self.g,sat_flow,7)
        ...
        #Check if in iso library
        graph_score=self.check_score_graph(ant_graph)
...
#            new=1
        ...
        return (ant_graph, graph_score) #,new
...
#                  fontsize=15)
        plt.title(r'Survivability$={:1.2f}$, Capacity$={:6.2f}$, Repeats={}'.format(score[0],score[1],score[2]),
                  fontsize=15)
                    ...
                    for i in range(0,2):
                     #systems
                        ...
                        if sat_flow[0]==1.0 and sat_flow[1]==1.0:
                            ...
                            graph_score=self.check_score_graph(added_ant)
...
#            l_perc_update=1-g_perc_update #ddpe invariance
...
if __name__=='__main__':
    ...
    while not converged:
        ...
        print (i,space.cycle_count,space.ant_failures,space.repeats),',',
